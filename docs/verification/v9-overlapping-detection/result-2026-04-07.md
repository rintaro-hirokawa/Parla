...
本検証では Azure Pronunciation Assessment をファイルベース（非ストリーミング）で使用しており、34秒の音声に対して 17秒の処理時間がかかっている。本番実装では `PushAudioInputStream` によるストリーミング送信を使用すること。ストリーミングでは音声をリアルタイムにチャンク送信し、Azure が並行処理するため、発話終了後の待ち時間は最後の utterance の処理待ち（~1秒）のみとなる。V11 で検証済み（[V11 結果](../v11-full-passage-evaluation/result-2026-04-08.md)）。
...
※ Azure の17秒は34秒の音声全体をファイルベース（非ストリーミング）で連続認識した処理時間。FA の1.4秒はタイムスタンプ取得のみ。Azure は発音評価を含む包括的な処理であり、合格基準の再定義が必要。なお、V11 でストリーミング方式（`PushAudioInputStream`）に切り替えたところ、体感レイテンシは ~1.2秒に短縮された（[V11 結果](../v11-full-passage-evaluation/result-2026-04-08.md) 参照）。本番実装ではストリーミング方式を採用すること。
